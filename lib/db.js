...
const dbOperations = {
  ...
  createGitlabProjectRequest: db.prepare(`
    INSERT INTO gitlab_project_requests (
      email, group_name, group_id, description, project_name, status,
      maintainers, developers, techStack, tags,
...
const dbHelpers = {
  ...
  createGitlabProjectRequest(data) {
    const result = dbOperations.createGitlabProjectRequest.run(
      ...
      data.project_name,
      data.status || 'pending',
      JSON.stringify(data.maintainers || []),
